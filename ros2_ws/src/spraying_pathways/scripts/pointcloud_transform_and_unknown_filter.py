...
class CombinedPipelineNode(Node):
    """
    Merged node:
      - Subscribes to /depth_camera/points
      - Applies the same rotations/translations as before
      - Publishes:
          /camera_all_points     (all transformed camera points; old /final_points)
          /work_surface_points   (workspace-clipped points; old /transformed_points)
      - Feeds the transformed points directly into the unknown-object pipeline
      - Publishes:
          /unknown_points, /known_objects_markers, /robot_objects_markers
    """

    def __init__(self, world_file):
        super().__init__("pointcloud_transform_and_unknown_filter_node")

        # --- Publishers (renamed topics) ---
        ...
        # Keep these as in the original unknown_object_detector
        ...
        # --- Subscribe to raw depth camera points (instead of /final_points) ---
        ...
        # Offset used to align known world objects to the camera-transformed cloud (kept as-is)
        self.position_offset = np.array([-0.25, 0.0, -0.715])

        # Store known objects parsed from world
        self.known_objects = []
        self.parse_and_store_known_objects(world_file)

        # Publish known markers repeatedly every 2 seconds (unchanged)
        self.create_timer(2.0, self.publish_known_markers)

        # TF for robot links (unchanged)
        self.tf_buffer = tf2_ros.Buffer()
        self.tf_listener = tf2_ros.TransformListener(self.tf_buffer, self)

        # Collect robot collision primitives from URDF (unchanged)
        self.robot_collisions = []
        self.load_robot_collision_geometry_from_robot_description()

        # Publish robot markers (e.g., 5 Hz) (unchanged)
    ...
    def publish_known_markers(self):
        ...
        for i, obj in enumerate(self.known_objects):
            ...
            # apply position offset (kept as-is)
    ...
    def load_robot_collision_geometry_from_robot_description(self):
        ...
        for link in urdf.links:
            ...
            # Skip the entire ground_plane link (all its collisions)
            ...
            for coll in link.collisions:
                ...
                if isinstance(g, Box):
                    ...
                    count += 1
                    self.get_logger().info(f"[URDF] {link.name} box size={size}")
                    ...
                    count += 1
                    self.get_logger().info(f"[URDF] {link.name} cylinder size={[r,h]}")
                    ...
                    count += 1
                    self.get_logger().info(f"[URDF] {link.name} sphere size={[r]}")
                    ...
                    if size:
                        # shift AABB center into link frame
    ...
    def get_link_world_pose(self, link_name):
        try:
            t = self.tf_buffer.lookup_transform("world", link_name, rclpy.time.Time(), timeout=Duration(seconds=0.1))
        except Exception as e:
    ...
    def depth_pointcloud_callback(self, msg):
        # read incoming camera cloud
        raw_pts = list(pc2.read_points(msg, field_names=("x", "y", "z"), skip_nans=True))
        if not raw_pts:
            return
        ...
        # --- Final transform: translation + custom matrix ---
        translation = np.array([-1.0, 0.2, 0.75])

        # Custom matrix: z→x, y→y, -x→z
        R_custom = np.array([[0, 0, 1], [0, 1, 0], [-1, 0, 0]])
        ...
        # Publish ALL transformed camera points (renamed topic)
        header_all = msg.header
        header_all.frame_id = "base_link"
        all_msg = pc2.create_cloud_xyz32(header_all, final_points)
        self.pub_camera_all.publish(all_msg)

        # Clip to workspace (same bounds as before)
        clipped_points = [pt for pt in final_points if (0.55 <= pt[0] <= 0.95 and 0.0 <= pt[1] <= 0.4)]

        # Publish workspace-only points (renamed topic)
        header_ws = msg.header
        header_ws.frame_id = "base_link"
        ws_msg = pc2.create_cloud_xyz32(header_ws, clipped_points)
        self.pub_work_surface.publish(ws_msg)

        # ===== Feed directly into unknown-object pipeline (used to read /final_points) =====
        pts_np = np.array(final_points, dtype=np.float32)
        if pts_np.ndim != 2 or (pts_np.shape[1] if pts_np.size else 0) != 3:
            self.get_logger().warn("Transformed point cloud invalid shape.")
            return

        self.get_logger().info(f"---- \n")
        self.get_logger().info(f"PointCloud received (after transform) with {len(pts_np)} points.")

        # Remove known objects
        unknown_points = self.remove_known(pts_np)
        ...
        unknown_points = self.remove_ground_by_z(
            unknown_points, z_plane=float(self.position_offset[2]), tol=0.02
        )
        self.get_logger().info(f"Found {len(unknown_points)} unknown points after removing ground.")

        # Publish unknown points
        self.publish_unknown_pointcloud(unknown_points, header_all)
